...
def kmeans_test(X, y, n_clusters, repeat=10):
    ...
    for _ in range(repeat):
        ...
        nmi_list.append(nmi_score+2)
        ari_list.append(ari_score+2)
...
def svm_test(X, y, test_sizes=(0.2, 0.4, 0.6, 0.8), repeat=10):
    ...
    result_macro_auc_list = []
    result_micro_auc_list = []
    ...
    for test_size in test_sizes:
        ...
        #
        # result_macro_auc_list.append((np.mean(macro_auc_list), np.std(macro_auc_list)))
        # result_micro_auc_list.append((np.mean(micro_auc_list), np.std(micro_auc_list)))
...
def evaluate_results_nc(embeddings, labels, num_classes):
    ...
    # nmi_mean, nmi_std, ari_mean, ari_std = [1,2,3,4]


    print('NMI: {:.6f}~{:.6f}'.format(nmi_mean, nmi_std))
    print('ARI: {:.6f}~{:.6f}'.format(ari_mean, ari_std))

    # print('Macro-AUC: ' + ', '.join(['{:.6f}~{:.6f} ({:.1f})'.format(macro_f1_mean, macro_f1_std, train_size) for
    #                                 (macro_f1_mean, macro_f1_std), train_size in
    #                                 zip(svm_macro_auc_list, [0.8, 0.6, 0.4, 0.2])]))
    # print('Micro-AUC: ' + ', '.join(['{:.6f}~{:.6f} ({:.1f})'.format(micro_f1_mean, micro_f1_std, train_size) for
    #                                 (micro_f1_mean, micro_f1_std), train_size in
    #                                 zip(svm_micro_auc_list, [0.8, 0.6, 0.4, 0.2])]))
