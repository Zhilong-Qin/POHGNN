...
class SHGNN_nc_layer(nn.Module):
    def __init__(self,
        ...
        self.ctr_ntype_layers = POHGNN_ctr_ntype_specific(num_metapaths, etypes_lists[0], in_dim)
    ...
    def forward(self, inputs):
        ...
        #最后聚合的h
        h_temp, temps= self.ctr_ntype_layers((features, type_mask, adj_matrixes, feature_idxes))
        h[np.where(type_mask == 0)[0]] = h_temp
        temp1,temp2=temps
        h[np.where(type_mask == 1)[0]]=temp1
        h[np.where(type_mask == 2)[0]] = temp2
        # fc_ 就算预测的fc
        if fc_switch:
...
class POHGNN(nn.Module):
    def __init__(self, num_layers, num_metapaths, etypes_lists, feats_dim_list, hidden_dim, out_dim, dropout_rate=0.5, adjD = None, count = None, count2= None):
        ...
        #建立 以feats_dim输入   hidden_dim输出的 神经网络
