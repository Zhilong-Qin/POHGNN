...
from model.POHGNN import POHGNN
import datetime
...
etypes_lists = [[[0, 1], [2, 3]],
                [[1, 0], [1, 2, 3, 0]],
                [[3, 2], [3, 0, 1, 2]]]  # 0-MD 1-DM 2-MA 3-AM
...
def run_model_IMDB(num_layers, hidden_dim, num_epochs, patience, repeat, save_postfix):
    #   matepath 实例，         特征            邻接矩阵   属于哪种节点  标签       寻测试
    ...
    for _ in range(repeat):
        net = POHGNN(num_layers, 2, etypes_lists,
                       [features_list[0].shape[1], features_list[1].shape[1], features_list[2].shape[1]], hidden_dim,
                       out_dim, dropout_rate, adjD, count, count2)
        net.to(device)
        optimizer = torch.optim.Adam(net.parameters(), lr=lr, weight_decay=weight_decay)
        target_node_indices = np.where(type_mask == 0)[0]
        # array([   0,    1,    2, ..., 4275, 4276, 4277], dtype=int64)   电影节点的索引
        # training loop
        net.train()
        ...
        # svm_macro_auc_lists.append(ma_auc)
        # svm_micro_auc_lists.append(mi_auc)
    # Evaluation
    ...
    # svm_macro_auc_lists = np.transpose(np.array(svm_macro_auc_lists), (1, 0, 2))
    # svm_micro_auc_lists = np.transpose(np.array(svm_micro_auc_lists), (1, 0, 2))
    # global global_log_file
    # global_log_file = 'output/IMDB/run_acdataset.log'
    print('----------------------------------------------------------------')
...
if __name__ == '__main__':
    ...
    ap.add_argument('--layers', type=int, default=1, help='Number of layers. Default is 2.')
    ...
    ap.add_argument('--data-set', default='IMDB', help='Dataset. Default is IMDB.')
